...
Libtorch_YOLO::Libtorch_YOLO(ros::NodeHandle node, torch::DeviceType di_type) : detector_(ros::package::getPath("libtorch_yolov5") + "/weights/best.torchscript", di_type)
{
    ...
    class_names_ = LoadNames(ros::package::getPath("libtorch_yolov5") + "/class.names");
    ...
    // subscribe
    subImg = node.subscribe<sensor_msgs::Image>("/usb_cam/image_raw", 1, &Libtorch_YOLO::imageCallback, this);
}
...
void Libtorch_YOLO::imageCallback(const sensor_msgs::ImageConstPtr &msg)
{
...
void Libtorch_YOLO::Demo(cv::Mat &img, const std::vector<std::vector<Detection>> &detections, const std::vector<std::string> &class_names, bool label)
{
    ...
    for (const auto &detection : detections[0]) 
    {
        ...
        // if(class_names[class_idx] != "person")
        // {
